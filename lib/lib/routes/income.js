...
router.findAll = function (req, res) {
    ...
    _incomes2.default.find(function (err, incomes) {
        if (err) res.send(err);
...
router.findOne = function (req, res) {
    ...
    _incomes2.default.find({ "_id": req.params.id }, function (err, income) {
        if (err) {
            res.status(404);
...
router.addIncome = function (req, res) {
    ...
    var income = new _incomes2.default();
...
router.deleteIncome = function (req, res) {
    ...
    _incomes2.default.findByIdAndRemove(req.params.id, function (err) {
        if (err) {
            res.status(404);
...
router.changeIncomeinfo = function (req, res) {
    ...
    _incomes2.default.findById(req.params.id, function (err, income) {
        if (err) {
            res.status(404);
...
router.findUserInByMessage = function (req, res) {
    ...
    _incomes2.default.find({ "email": req.params.email, "message": { $regex: req.params.message, $options: 'i' } }, function (err, income) {
...
router.findUserMonthIncome = function (req, res) {
    ...
    res.setHeader('Content-Type', 'application/json');
    //var keyword = {'date': {$regex:req.params.date, $options:'i'}};
    _incomes2.default.find({ "email": req.params.email, "date": { $regex: req.params.date, $options: 'i' } }, function (err, incomes) {
